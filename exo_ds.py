...
def relation_extraction(train,
    ...
    for n_batches in used_train_batches[-1:]:
        ...
        classifier = MLPDropout(rng, input=layer1_input,
                                ...
                                dropout_rates=dropout_rate, W=MLP_W, b=MLP_b)
        ...
        train_predict = predict_relation(train_rels, train_nums, train_sents,
                                         train_poss, train_eposs, test_one,
                                         img_h)

        f_res = open(datadir + "/re_results.txt", "w")
        for pred_y, pred_y_prob, label, sig_eposs,sig_sent \
                in zip(*train_predict):
            res = ' '.join(map(lambda x: str(x), sig_eposs)) + '\t' + \
                        ' '.join(map(lambda x: str(x), sig_sent)) + '\t' + \
                        str(pred_y) + '\t' + str(pred_y_prob) + '\t' + \
                        '_'.join(map(lambda x: str(x), label)) + '\n'
            f_res.write(res)
        f_res.close()
        ...
        now = time.strftime("%Y-%m-%d %H:%M:%S")
        print str(now) + '\t epoch ' + str(15) + ' test set PR = [' + str(test_pr[0][-1]) + ' ' + str(
            test_pr[1][-1]) + ']'


        test_pr = positive_evaluation(train_predict)
        now = time.strftime("%Y-%m-%d %H:%M:%S")
...
def train_conv_net(train,
    ...
    for n_batches in used_train_batches[-1:]:
        ...
        while (epoch < epochs):
            ...
            if curriculum == "none":
                #TODO: change 100 to n_batches
                for train_batch_idx in range(n_batches):
                    if (train_batch_idx % 4000 == 0) or \
                            (train_batch_idx == n_batches - 1):
        ...
        if not os.path.exists(datadir + "/trained_data/"):
            os.mkdir(datadir + "/trained_data/")
        pickle.dump(list(map(lambda x: x.get_value(), params)),
                    open(datadir + "/trained_data/params.p", "wb"))
    return list(map(lambda x: x.get_value(), params))
...
def predict_relation(rels, nums, sents, poss, eposs, test_one, img_h):
    ...
    print '[' + time.asctime(time.localtime()) + "] total: " + str(numBags)
    for bagIndex, insRel in enumerate(rels):
        if bagIndex%10000 == 0:
            print '[' + time.asctime(time.localtime()) + "] index: " + str(bagIndex)
...
if __name__ == "__main__":
    ...
    if not os.path.exists('./results_exo/'):
        os.mkdir('./results_exo/')

    # change result dir name as you want
    if pretrain == 'none':
        resultdir = './results_exo/' + str(pretrain) + '_r_' + str(rnd)
    else:
        resultdir = './results_exo/' + str(pretrain) + '_r_' + str(rnd) + '_opt_' + \
    ...
    if os.path.exists(inputdir + "/trained_data/params.p"):
        MLP_W, MLP_b, conv_layer_W, conv_layer_b, Wv, PF1, PF2 = pickle.load(
            open(inputdir + "/trained_data/params.p", "rb"))
    else:
        print '[' + time.asctime(time.localtime()) + "] train_conv_net started..."
        MLP_W, MLP_b, conv_layer_W, conv_layer_b, Wv, PF1, PF2 = \
