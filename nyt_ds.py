...
def pre_train_conv_net(train,
    ...
    [valid_nums, valid_sents, valid_poss, valid_eposs] = pre_train_bags_decompose(new_valid)

    if pretrain == Pretrain.SKIPGRAM:
        train_model_batch = theano.function([x, p1, p2, pool_size, context, neg], cost, updates=grad_updates)
        valid_model_batch = theano.function([x, p1, p2, pool_size, context, neg], cost)
    elif pretrain == Pretrain.DEPSP:
        train_model_batch = theano.function([x, p1, p2, pool_size, context, context_msk, neg], cost, updates=grad_updates)
        valid_model_batch = theano.function([x, p1, p2, pool_size, context, context_msk, neg], cost)
    else:
        train_model_batch = theano.function([x, p1, p2, pool_size, di, dm, dt, neg], cost, updates=grad_updates, on_unused_input='ignore')
        valid_model_batch = theano.function([x, p1, p2, pool_size, di, dm, dt, neg], cost, on_unused_input='ignore')
    ...
    cost_f = open(directory + "/pre_trained_data/cost_log_epoch", "w")
    while (epoch < epochs):
        print("\n### epoch " + str(epoch) + " ###")
        if curriculum == "none":
            for train_batch_idx in range(n_train_batches):
                ...
                if pretrain == Pretrain.SEQ2SEQ:
                    ...
                    train_model_batch(x, p1, p2, pool_size, deIpt, deMst, deTgt, neg_idx)
                else:
                    ...
                    if pretrain == Pretrain.SKIPGRAM:
                        train_model_batch(x, p1, p2, pool_size, context_idx, neg_idx)
                    else:
                        train_model_batch(x, p1, p2, pool_size, context_idx, context_msk, neg_idx)
                set_zero(zero_vec)
        ...
        for valid_batch_idx in range(n_valid_batches):
            inst_indices = range(valid_batch_idx * batch_size,
                                 (valid_batch_idx + 1) * batch_size)
            v_x, v_p1, v_p2, v_pool_size, v_context_idx = pre_train_get_batch_data(
                    inst_indices, valid_nums, valid_sents, valid_poss,
                    valid_eposs, img_h, ctx_size)
            if pretrain == Pretrain.SEQ2SEQ:
                deIpt = valid_decoder_inputs[:, inst_indices]
                deTgt = valid_decoder_targets[:, inst_indices]
                deMst = utils.get_mask(deIpt)
                neg_idx = np.asarray(output_layer.table.sample(deIpt.shape[0]*batch_size*10, for_test), dtype='int32')\
                    .reshape((deIpt.shape[0], batch_size, 10))
                cost += valid_model_batch(v_x, v_p1, v_p2, v_pool_size,
                                  deIpt, deMst, deTgt, neg_idx)
            else:
                if pretrain == Pretrain.DEPSP:
                    context_idx = np.asarray(
                        [valid_dep_sp_ctx[m] for m in inst_indices],
                        dtype='int32')
                    context_msk = utils.get_mask(context_idx)
                neg_num = context_idx.shape[1] * 10
                neg_idx = np.asarray(output_layer.table.sample(batch_size * neg_num, for_test), dtype='int32') \
                    .reshape((batch_size, neg_num))
                if pretrain == Pretrain.SKIPGRAM:
                    cost += valid_model_batch(v_x, v_p1, v_p2, v_pool_size, v_context_idx, neg_idx)
                else:
                    cost += valid_model_batch(v_x, v_p1, v_p2, v_pool_size, context_idx, context_msk, neg_idx)
        cost_f.write(str(epoch) + " " + str(cost) + "\n")
    cost_f.close()
...
if __name__ == "__main__":
    ...
    if pretrain == 'none':
        resultdir = './results_' + res_dir_pre + '/' + str(pretrain) + '_r_' + str(rnd)
    else:
        resultdir = './results_' + res_dir_pre + '/' + str(pretrain) + '_r_' + str(rnd) + '_opt_' + \
